class Connection {
  ...
  connectActions() {
    console.log("REACTIVE OBSERVER LOGGED IN")
    /// REFRESH OBSERVABLES!
  ...
  preObserveAction () {
    return new Promise((resolve, reject) => {
      this.preObserveRequest()
        .then(() => resolve())
        .catch(e => {
          this.middleTimeout = setTimeout(() => {
            this.preObserveAction().then(resolve)
          }, 5e3)
        })
    })
  }
  ...
  handleConnect() {
    console.log("REACTIVE OBSERVER CONNECTED")
    this.connected = true
    this.send({
